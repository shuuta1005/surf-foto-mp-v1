...
import soundWaveGif from "@/assets/sound-wave.gif";
...
const DestinationsGrid = ({ galleries }: DestinationsGridProps) => {
  ...
  const handleAreaClick = (area: string) => {
    setLoadingArea(area); // ✅ mark clicked area as loading
        ...
        {isMobile ? (
          <div className="flex overflow-x-auto gap-4">
            {destinationsToDisplay.map((gallery, index) => (
              ...
              >
                <div className="w-full h-28 sm:h-32 relative mb-2 flex items-center justify-center">
                  {loadingArea === gallery.area ? (
                    <Image
                      src={soundWaveGif} // 👈 replace with your gif path
                      alt="Loading"
                      width={200}
                      height={200}
                    />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {destinationsToDisplay.map((gallery, index) => (
              ...
              >
                <div className="w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 relative flex items-center justify-center">
                  {loadingArea === gallery.area ? (
                    <Image
                      src={soundWaveGif} // 👈 replace with your gif path
                      alt="Loading"
